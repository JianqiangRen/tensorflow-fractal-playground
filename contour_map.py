# coding=utf-8
# summary: generate contour map like background
...
if __name__ == '__main__':
    ...
    for zs_exp in range(2, 6):
        for xs_exp in range(1, 4):
            for c_1 in [x * 0.2 + 1 for x in range(1, 10)]:
                for c_2 in [x * 0.2 + 1 for x in range(1, 10)]:
                    ...
                    if not is_pure(img_a):
                        cv2.imwrite("contour/contour_{}_{}_{:.2f}_{:.2f}_a.png".format(zs_exp, xs_exp, c_1, c_2), img_a)
                    
                    img_b = img[100: 100 + target_height, 0:target_width, :]
                    if not is_pure(img_b):
                        cv2.imwrite("contour/contour_{}_{}_{:.2f}_{:.2f}_b.png".format(zs_exp, xs_exp, c_1, c_2), img_b)
                    
                    img_c = img[h - target_height: h, 0: target_width, :]
                    if not is_pure(img_c):
                        cv2.imwrite("contour/contour_{}_{}_{:.2f}_{:.2f}_c.png".format(zs_exp, xs_exp, c_1, c_2), img_c)
